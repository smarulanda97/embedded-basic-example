...
/// Structure representing the LEDs matrix
...
impl<'a, L: Led> SyscallDriver for GlyphDisplay<'a, L> {
    fn command(&self, command_number: usize, r2: usize, _r3: usize, _process_id: ProcessId) -> CommandReturn {
        match command_number {
            0 => CommandReturn::success(), // To confirm that driver is present
            1 => match self.display(r2 as u8 as char) {
                Ok(()) => CommandReturn::success(),
                Err(err) => CommandReturn::failure(err),
            },
            _ => CommandReturn::failure(ErrorCode::NOSUPPORT), // Unsupported command
        }
    }
